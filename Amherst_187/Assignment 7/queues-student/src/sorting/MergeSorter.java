...
public class MergeSorter<T extends Comparable<T>> {
	...
	public Queue<T> mergeSort(Queue<T> queue) {
		...
		Queue<T> output1 = new Queue<T>();  // create new outputs each call to store our divisions in
		Queue<T> output2 = new Queue<T>();
		if (q.size() > 1) {  // if the queue is size 1 or 0, it is already sorted
			divide(q, output1, output2); // divide queue into 2 output queues
			// The inputs start merging once they reach the base case
			// They keep getting split until they end up at either 1 or 0 elements
			Queue<T> input1 = mergeSort(output1); // input1 keeps calling until it reaches 1 or 0 element
			Queue<T> input2 = mergeSort(output2); // input2 keeps calling until it reaches 1 or 0 element
			// Then they merge back together until it reaches back to the scope where it started the call
			// Every call results into a sorted queue until it hits the top scope, where it returns the a new queue that is a sorted version of the initial queue 
			return merge(input1, input2); // merge the 2 inputs to get a sorted queue -> keeps repeating until it reaches the final scope or when the whole queue is sorted
		} else { // base case -> queue is of size 0 or 1 -> return the queue
			return q;
		}
	}
