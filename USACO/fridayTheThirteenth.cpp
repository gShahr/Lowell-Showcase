...
int main() {
  ...
  vector<int> increment = {3, 0, 3, 2, 3, 2, 3, 3, 2, 3, 2, 3}; // how much each month pushes the days forward
  ...
  for (int i = 1900; i < 1900+years; i++) { // iterates through the years starting from 1900 until the amount of years the user inputs
    for (int j = 0; j < 12; j++) { // iterate through the months
      ...
      else {start += increment[j];} // add the increment based on how much each month pushes forward the days
    }
  }
