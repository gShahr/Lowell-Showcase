...
// Preconditions
   // 1. Quiz file must exist
   // 2. Name must be less than 20 characters
   // 3. Student must have 10 or less quiz scores
   // 4. Nothing else besides name and quiz scores should be on the files
...
void is_valid(FILE *input1, FILE *output) {
  ...
  if (white_space < 2) {
    printf("Invalid Name!"); // invalid name due to it being too long (error message)
...
void format_data(FILE *input1, FILE *output) {
  ...
  printf("\nThere are %d students", lines); // used to debug
